...
2. `core/style.py` - ANSI-стили с гарантированным RESET
3. `core/base.py` - BaseModule абстракция
4. `core/config.py` - загрузка TOML конфига
5. `core/loader.py` - загрузка встроенных модулей по конфигу
6. `modules/model.py` - модель + контекст + duration
7. `__init__.py` - main() связывает всё вместе

### Модуль git (v0.2)
8. `modules/git.py` - директория + ветка + статус + изменения

### Модуль beads (v0.3)
9. `modules/beads.py` - дерево задач из .beads/

### Модуль quota (v0.4)
10. `modules/quota.py` - квота API (session/weekly), кэширование
...
Изменения:
- Переименование: statusline → statuskit
- Добавлен roadmap с декомпозицией
- Monorepo структура с uv workspace
