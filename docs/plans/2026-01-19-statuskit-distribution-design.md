...
**Gitignore для local scope:**

При установке в local scope (`--scope local`) statuskit проверяет, что local файлы игнорируются git'ом.

Логика:
1. Проверить, что мы в git-репозитории (`git rev-parse --git-dir`)
2. Если не в репо — пропустить gitignore логику
3. Проверить каждый файл через `git check-ignore -q`:
   - `.claude/settings.local.json`
   - `.claude/statuskit.local.toml`
4. Если оба уже игнорируются — ничего не делаем
5. Если хотя бы один не игнорируется:
   - Создать `.gitignore` если не существует
   - Добавить `.claude/*.local.*` в конец файла
...
Если файлы уже игнорируются — ничего не показываем.
