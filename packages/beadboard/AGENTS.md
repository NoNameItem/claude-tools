...
**Layering (P1).** A layer may not import a layer above it, and only `ui` may import
`textual`. The rule is enforced by `tests/test_import_boundaries.py` against the `FORBIDDEN`
table — a change that edits that table to make a new import legal is a design change and needs
a reason in the pull request, not a green test.
